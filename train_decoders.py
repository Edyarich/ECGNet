...
eeg_lp = rootpath + 'eeg_data/' # path to EEG xarray data
eeg_roi_proj_lp = rootpath + 'proj-matrices-eeg/' # path to EEG projection matrix
...
for s,val in enumerate(spec_meas_tail):
    ...
    if s==0:
        models = ['eegnet_hilb', 'eegnet'] # ,'eegnet','rf','riemann'] # fit all decoder types
    else:
